    ...
    public class FairyTargets {
        ...
        public static void CreateFairyTargets() {
            ...
            if (ItemLookup.ItemList.Count > 0) {
                List<string> ItemIdsInScene = Locations.VanillaLocations.Keys.Where(ItemId => Locations.VanillaLocations[ItemId].Location.SceneName == SceneLoaderPatches.SceneName && SaveFile.GetInt($"randomizer picked up {ItemId}") == 0).ToList();
                if (ItemIdsInScene.Count > 0) {
                    ...
                    if (GameObject.FindObjectOfType<TrinketWell>() != null) {
                        ...
                        int CoinsNeededForNextReward = CoinLevels[Locations.VanillaLocations.Keys.Where(ItemId => Locations.VanillaLocations[ItemId].Location.SceneName == "Trinket Well" && SaveFile.GetInt($"randomizer picked up {ItemId}") == 0).ToList().Count];
