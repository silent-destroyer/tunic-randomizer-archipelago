    ...
    public class SpeedrunFinishlineDisplayPatches {
        ...
        public static bool SpeedrunFinishlineDisplay_addParadeIcon_PrefixPatch(SpeedrunFinishlineDisplay __instance, ref Sprite icon, ref int quantity, ref RectTransform rt) {
            if (icon.name == "Inventory items_money triangle") {
                if (TunicArchipelago.Tracker.ImportantItems["Golden Trophies"] == 12) {
                    quantity = 1;
                    return true;
                } else {
                    return false;
            ...
            if (TunicArchipelago.Tracker.ImportantItems[ReportGroupItems[icon.name]] == 0) {
                return false;
            }

            quantity = TunicArchipelago.Tracker.ImportantItems[ReportGroupItems[icon.name]];
            return true;
        }
        ...
        public static void GameOverDecision_Start_PostfixPatch(GameOverDecision __instance) {
            int MissingPageCount = (28 - TunicArchipelago.Tracker.ImportantItems["Pages"]);
